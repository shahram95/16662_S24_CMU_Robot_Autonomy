...
def CheckPointOverlap(pointsA, pointsB, axis):	
    ...
    # Normalize the axis to avoid distortion in projection scale
    normalized_axis = axis / np.linalg.norm(axis)
    
    # Project points of both boxes onto the axis
    projectionA = np.dot(pointsA, normalized_axis)
    projectionB = np.dot(pointsB, normalized_axis)
    
    # Find the min and max projection values for each box
    minA, maxA = np.min(projectionA), np.max(projectionA)
    minB, maxB = np.min(projectionB), np.max(projectionB)
    
    # Check for overlap
    overlap = (minA <= maxB) and (minB <= maxA)
...
def CheckBoxBoxCollision(pointsA, axesA, pointsB, axesB):
    ...
    for axisA in axesA:
        for axisB in axesB:
            # Skip parallel axis pairs to avoid division by zero in cross product
            if np.linalg.norm(np.cross(axisA, axisB)) > 1e-6:  
                if not CheckPointOverlap(pointsA, pointsB, np.cross(axisA, axisB)):
                    return False
